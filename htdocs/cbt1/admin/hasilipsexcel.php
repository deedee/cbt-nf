<?php
	...
	$tempExcel = \PhpOffice\PhpSpreadsheet\IOFactory::load("templateExcel/TemplateHasilIPS.xlsx");
	...
	//cari ganti nama bidang studi IPS BS->
	$BSname = array();
	for ($i=1; $i < 11; $i++) { 
		$BSname['BS'.$i]='';
	}

	$dapatBS = mysqli_query($con,"SELECT * FROM tabelhasil WHERE kelompok='IPS'");
	...
	$dapatSiswa = mysqli_query($con,"SELECT * FROM hasilips");
	...
	while ($dataSiswa = mysqli_fetch_array($dapatSiswa))
	{
		...
		$carihp = "SELECT piljur1, piljur2, piljur3, nomorHP, alamatEmail, tglLahir FROM absensiharitespeserta WHERE nomorInduk='$nI' AND nomorPeserta='$nP'";
	...
	header('Content-Disposition: attachment;filename="Hasil TO CBT - IPS.xlsx"');
